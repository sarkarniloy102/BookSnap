...
const Hero = ({ searchQuery }) => {
    ...
    useEffect(() => {
        const fetchBooks = async () => {
            try {
                setLoading(true);
                const response = await fetch(`https://www.googleapis.com/books/v1/volumes?q=${encodeURIComponent(searchQuery)}&maxResults=40&key=AIzaSyDglloUiS0VAXgz-amFsZtdSVk554sRxfM`);
                ...
                let limitedBooks = mappedBooks.slice(0, 40);
                if (limitedBooks.length < 40) {
                    const missingCount = 40 - limitedBooks.length;
                ...
                const dummyBooks = Array.from({ length: 40 }, (_, i) => ({
                    dummy: true,
                    id: `dummy-${i}`
                }))
                setBooks(dummyBooks);
        ...
        return (
            <div className="min-h-screen bg-gradient-to-b from-gray-900 to-gray-800 p-8 flex justify-center items-center">
                <p className="text-cyan-300 text-lg md:text-xl text-center">
                    No Books Found Matching In Your Search.
                </p>
